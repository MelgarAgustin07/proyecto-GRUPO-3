...
# [AJUSTE] Dimensiones para Nivel MEDIO: 550x580 px
ANCHO_VENTANA = 550 
ALTO_VENTANA = 580

# [AJUSTE] Lógica del Juego: 10 pares (20 cartas) en cuadrícula 4x5
TOTAL_PARES = 10 
FILAS = 5 
COLUMNAS = 4
...
def main(ventana_principal, funcion_actualizar_records):
    ...
    try:
        # Usamos el nombre de imagen para el nivel Medio
        ruta_fondo_medio = os.path.join(ruta_imagenes, "imagen_medio.png") 
        
        # Carga la imagen, redimensiona y guarda la referencia
        img_pil = Image.open(ruta_fondo_medio)
    ...
    # Posición para centrar el frame de botones en el nuevo alto (580px)
    centro_x = ANCHO_VENTANA / 2
    centro_y_botones_ajustado = 300 # Ajuste visual para 580px de alto
    ...
    # Lista de imágenes (Asegúrate de que tengas al menos 10 imágenes distintas)
    Imagenes = [
    ...
    # Agrega las imágenes necesarias aquí si tu carpeta tiene más de 10
    ]

    imagen_reverso = tk.PhotoImage(file=os.path.join(ruta_imagenes, "reverso.png"))

    # Seleccionamos 10 pares (TOTAL_PARES) de imágenes.
    ...
    # Recorremos 20 cartas (TOTAL_PARES * 2)
    for i in range(TOTAL_PARES * 2):
        ...
        # Usamos las constantes de FILAS y COLUMNAS para la cuadrícula 4x5
